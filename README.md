...
- **반응형 레이아웃**
  - 웹(768px+): 폴더 트리 사이드바 + 링크 카드 그리드
  - 모바일: 방사형 버블 + drill-down

## 주요 기능

- **폴더 트리** — 무제한 깊이, 생성·이름변경·이동·삭제
...
- `src/components` — TreeView·LinkCardList·LinkCard(웹), RadialBubbleView(모바일), YouTubePlayer, InputModal
- `src/hooks` — `useCategories`, `useLinks`, `useMediaQuery`
- `src/styles` — `global.css` (CSS 변수 기반 다크 테마)
